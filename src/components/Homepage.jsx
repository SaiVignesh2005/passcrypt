...
const Homepage = () => {
  ...
  const handleSavePassword = async () => {
    ...
    const payload = { ...userInfo, id: userInfo.id || uuidv4() };

    if (userInfo.id) {
      const res = await fetch(`http://localhost:3000/passwords/${userInfo.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      res.ok && showToast('Password updated!');
    } else {
      const res = await fetch('http://localhost:3000/passwords', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      res.ok && showToast('Password saved!');
    }
                                        ...
                                        <td className="py-2 px-4 text-left">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <button onClick={() => copyText(info.site)} className="flex-shrink-0">
                                                    <img src={copy} alt="Copy site" className="w-5 h-5" />
                                                </button>
                                            </div>
                                        </td>
